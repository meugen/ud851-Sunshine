...
//      COMPLETED (9) Handle queries on both the weather and weather with date URI
        ...
        switch (match) {
            ...
            case CODE_WEATHER_WITH_DATE:
                final String _selection = WeatherContract.WeatherEntry.COLUMN_DATE + "=?";
                final String[] _selectionArgs = new String[] {
                        uri.getPathSegments().get(1)
                };
                cursor = db.query(WeatherContract.WeatherEntry.TABLE_NAME,
                        projection, _selection, _selectionArgs, sortOrder, null, null);
                break;
            default:
