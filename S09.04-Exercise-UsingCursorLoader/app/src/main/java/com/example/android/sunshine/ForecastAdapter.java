...
//  COMPLETED (3) Add a Context field to the constructor and store that context in mContext
     ...
     */
    public ForecastAdapter(final Context context, ForecastAdapterOnClickHandler clickHandler) {
        mContext = context;
...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        final String dateStr = SunshineDateUtils.getFriendlyDateString(mContext,
                mCursor.getLong(MainActivity.INDEX_DATE), true);
        final String weatherStr = SunshineWeatherUtils.getStringForWeatherCondition(mContext,
                mCursor.getInt(MainActivity.INDEX_WEATHER_ID));
        final String tempStr = SunshineWeatherUtils.formatHighLows(mContext,
                mCursor.getDouble(MainActivity.INDEX_MAX_TEMP),
                mCursor.getDouble(MainActivity.INDEX_MIN_TEMP));
        final String weatherSummary = dateStr + " - " +
                weatherStr + " - " + tempStr;
